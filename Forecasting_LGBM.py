# -*- coding: utf-8 -*-
"""
Created on Tue Jan 19 10:51:55 2021
...
best_params_lg=pd.DataFrame((space_eval(space,best)).values())

model2=LGBMRegressor(random_state=random_state, learning_rate=best_params_lg[0][0],max_depth=int(best_params_lg[0][1]),
                             n_estimators=int(best_params_lg[0][2]))
...
def llamar_modelo(X,y,test_size=0.11):
    ...
    model=LGBMRegressor(learning_rate=best_params_lg[0][0],max_depth=int(best_params_lg[0][1]),
                             n_estimators=int(best_params_lg[0][2]))
...
writer=pd.ExcelWriter('Proyección_LGBM.xlsx',engine='xlsxwriter')
