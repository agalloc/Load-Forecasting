# -*- coding: utf-8 -*-
"""
Created on Tue Jan 19 10:51:56 2021
...
space={'max_depth': hp.quniform('max_depth', 1,50,1),
       ...
       'criterion': hp.choice('criterion', ['squared_error', 'friedman_mse', 'mae'])}
#Historial de evaluación 
trials=Trials()
##
best=fmin(fn=opt_r2,space=space,algo=tpe.suggest,max_evals=n_iter,trials=trials)
#Almacenar mejores hiperparámetros

best_params_dt=pd.DataFrame((space_eval(space,best)).values())

model2=DecisionTreeRegressor(random_state=random_state, criterion=best_params_dt[0][0],max_depth=int(best_params_dt[0][1]),
                             max_features=int(best_params_dt[0][2]))
score2=-cross_val_score(model2,train_data,train_targets,cv=kf,scoring='r2',n_jobs=-1).mean()
...
#Separa los sets de entrnamiento y prueba
...
def llamar_modelo(X,y,test_size=0.11):
    ...
    #Definir el algoritmo de aprendizaje supervisado 
    model=DecisionTreeRegressor(criterion=best_params_dt[0][0],max_depth=int(best_params_dt[0][1]),
                                max_features=int(best_params_dt[0][2]))
...
#Proyectar la demanda de la siguiente semana
...
plt.title('Porcentaje de Error Medio Absoluto {0:.2f}%'.format(error))
plt.legend(loc='best')
plt.tight_layout()
plt.grid(True)
...
writer=pd.ExcelWriter('Proyección_DT.xlsx',engine='xlsxwriter')
